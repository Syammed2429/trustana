...
test.describe('Product Filters', () => {
  ...
  test('should navigate to product detail page', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="product-overview"]')).toBeVisible();
  });
  ...
  test('should handle large datasets with infinite scroll', async ({ page }) => {
  ...
  test('should handle error states gracefully', async ({ page }) => {
    ...
    await expect(noResultsText.or(emptyState).or(page.locator('text="No results"'))).toBeVisible({
      timeout: 5000,
    });
  });
});
