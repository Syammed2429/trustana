...
interface FilterConditionItemProps {
  ...
  onDataTypeChange: (dataType: 'string' | 'number' | 'boolean' | 'date' | 'price' | 'url') => void;
  onOperatorChange: (operator: keyof InternalFilterValue) => void;
  onValueChange: (value: string | boolean) => void;
  onRemove: () => void;
}

export const FilterConditionItem: FC<FilterConditionItemProps> = memo(function FilterConditionItem({
  condition,
  availableAttributes,
  ...
  return (
    <div className='flex items-center gap-2 rounded-lg border p-3'>
      {showLogicalOperator && (
        <div className='text-muted-foreground min-w-[30px] text-xs font-medium'>
          {logicalOperator}
        </div>
      )}
      ...
      <Button size='sm' variant='outline' onClick={onRemove} className='shrink-0'>
        <X className='h-4 w-4' />
      </Button>
    </div>
  );
